...
from taskmanager.forms import TaskNameSearchForm, WorkerUsernameSearchForm, TaskTypeNameSearchForm, \
    PositionNameSearchForm, WorkerSortForm, TaskSortForm, PositionSortForm, TaskTypeSortForm, LoginForm, \
    UserPasswordChangeForm, WorkerCreationForm, WorkerChangeForm, TaskChangeForm
...
def index(request) -> HttpResponse:
    context = {
        ...
        'nearest_deadline': Task.objects.filter(is_complete=False).order_by('deadline')[0],
    }
...
class TaskListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(
        ...
        sort_value = self.request.GET.get('sort_by')
        context['sort_form'] = TaskSortForm(initial={
    ...
    def get_queryset(self):
        ...
        sort_by = self.request.GET.get('sort_by')
        if sort_by in ['id', 'name', 'deadline', 'is_complete', 'priority', 'task_type']:
...
class WorkerListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(
        ...
        context['sort_form'] = WorkerSortForm(initial={
            'sort_by': self.request.GET.get('sort_by', '')
        })
...
class TaskTypeListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(
        ...
        sort_value = self.request.GET.get('sort_by')
        context['sort_form'] = TaskTypeSortForm(initial={
            'sort_by': self.request.GET.get('sort_by', '')
        })
...
class PositionListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(
        ...
        sort_value = self.request.GET.get('sort_by')
        context['sort_form'] = PositionSortForm(initial={
            'sort_by': self.request.GET.get('sort_by', '')
        })
